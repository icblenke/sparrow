module AQueue
  class Queue
    class AQueueError < StandardError; end #:nodoc:
    class NoServersLeft < AQueueError; end #:nodoc:
    
    PID_DIR = File.join(AQUEUE_ROOT, 'tmp', 'pids')
    ...
    class << self
      ...
      def send_to_server(random = false, &block)
        ...
        srvs.each do |server|
          begin
            return yield(server)
          rescue AQueue::Protocols::ConnectionError
